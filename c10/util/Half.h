...
#include <c10/util/FloatingPointUtils.h>
...
inline uint32_t fp16_to_fp32_bits(uint16_t h) {
  /*
...
// 0 00000001 0000 0000 0000 0000 000 + (0111 0000 - 5) = 0110 1011 << 23 =
// 0 01101011 0000000000000000000000
// 0 01101011 0000 0000 0000 0000 000
// 0 01101100 0000 0000 0000 0000 000
// 当我们左移尾数时，数值本身被放大了2^renorm_shift倍
// 为了抵消这个放大效果，需要在指数上减去相同的量，使得整体数值保持不变。

C10_HOST_DEVICE inline float fp16_to_fp32_value(uint16_t h) {
  /*
   * Extend the half-precision floating-point number to 32 bits and shift to the
   * upper part of the 32-bit word:
   *      +---+-----+------------+-------------------+
...
inline uint16_t fp16_from_fp32_value(float f) {
